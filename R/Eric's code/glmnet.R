...
load("../age_and_methylation.rdata")
...
# multiple runs of the best glmnet model (alpha at lowest cvm)

glmnet.tune.best <- parallel::mclapply(1:nrep, function(i) {
  foldid <- sample(1:nfolds, size = nrow(model.ages), replace = TRUE)
  cv.glmnet(
    x = as.matrix(model.ages[, sites.to.keep]),
    y = model.ages$age.best,
    weights = model.ages$confidence.wt,
    alpha = min.median.cvm$alpha,
    nfolds = nfolds,
    foldid = foldid
  ) 
}, mc.cores = 10)

lambda.1se <- sapply(glmnet.tune.best, function(x) x$lambda.1se) 
hist(lambda.1se)
