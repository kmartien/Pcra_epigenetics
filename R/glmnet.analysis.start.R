...
load(paste("MS15.galaxy.alignments/",description,".pct.meth.Rdata",sep=""))
...
#age.data <- read.csv("Pcra.age.estimates.csv")
age.data <- read.csv("eia.datasheet.1.csv")
...
#x.meth <- as.matrix(calibration.set.complete[,9:209])
...
#x.meth <- as.matrix(calibration.set.complete[,8:208])
...
#  return(list(corr.coef=corr.coef,predicted.decade=predicted.decade))
...
#################################################
# Cross-validated multinomial fit of social cluster over a range of alphas

#x.meth <- as.matrix(calibration.set.complete[,8:208])
y.cluster <- as.matrix(calibration.set.complete$social.cluster)
to.remove <- which(y.cluster[,1] %in% c("IC","2*"))
y.cluster <- as.numeric(y.cluster[-to.remove,])
x.meth.cluster <- x.meth[-to.remove,]

test.alpha.cluster <- lapply(alpha.vals, function(a){
  cvfit <- cv.glmnet(x.meth.cluster,y.cluster, alpha = a, family="multinomial")
  corr.coef <- coef(cvfit, s = "lambda.min")
  predicted.cluster <- predict(cvfit, x.meth, s = "lambda.min", type="class")
  #  return(list(corr.coef=corr.coef,predicted.decade=predicted.decade))
  return(list(corr.coef=corr.coef,predicted.cluster=predicted.cluster))
})

cluster.sum <- lapply(test.alpha.cluster, function(a){
  cbind(y.cluster, a$predicted.cluster)
})

##################################################
#  Random Forest classification of social clusters

sampsize <- rep(ceiling(min(table(y.cluster) / 2)), length(table(y.cluster)))
cluster.rf <- as.factor(y.cluster)
